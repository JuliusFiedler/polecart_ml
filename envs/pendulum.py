...
class PendulumEnv(BaseEnv):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def get_reward(self):
        th, thdot = self.state
        costs = - angle_normalize(th) ** 2 + 0.1 * thdot**2 + 0.001 * (self.last_u**2)
        ...
        return costs, terminated, truncated, info

    def post_processing_state(self):
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None, state=None):       
        ...
        if options is None:
            if hasattr(self.c, "reset_options"):
                x = self.c.reset_options.get("x_init")
                y = self.c.reset_options.get("y_init")
    ...
    def render(self):
        ...
        p = precision = 3
        x = self.state
        u.text_to_screen(self.surf, f"theta {np.round(x[0], p)}", (int(self.screen_width / 2), 10))
        u.text_to_screen(self.surf, f"omega {np.round(x[1], p)}", (int(self.screen_width / 2), 30))
        if self.reward is not None:
            u.text_to_screen(self.surf, f"Rew {np.round(self.reward, p)}", (int(self.screen_width / 2), 70))
        if self.action is not None:
            u.text_to_screen(self.surf, f"Act {np.round(self.action, p)}", (int(self.screen_width / 2), 90))
...
class StdPendulumEnv(PendulumEnv):
    def __init__(self, render_mode: str = None):
        super().__init__(render_mode)
        import envs.parameter.PendulumEnv as c
        self.c = c
